...
- Legacy `nodes[].nbt`: optional SNBT string matched against submitted stacks, for example `{Damage:0}` or `{"Potion":"minecraft:water"}`. The specified NBT must be present; unrelated extra NBT on the submitted stack is allowed. For new `ingredient` nodes, prefer the loader's own custom ingredient format instead.
